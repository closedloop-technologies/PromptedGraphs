...
def is_potentially_unsafe(node):  # sourcery skip: low-code-quality
    if isinstance(node, ast.Call):
        ...
        for stmt in node.body:
            if isinstance(stmt, ast.Expr) and isinstance(stmt.value, ast.Call) and (isinstance(stmt.value.func, ast.Name) and stmt.value.func.id == 'exec'):
                return True
    elif isinstance(node, ast.Assign):
        # Disallow assignments of potentially unsafe values
        for target in node.targets:
            if isinstance(target, ast.Name) and target.id in {'__builtins__', 'eval', 'exec', 'open'}:
                return True
...
def safer_exec(model_code):
    ...
    safe_code_body = [node for node in parsed_code.body if not is_potentially_unsafe(node)]
